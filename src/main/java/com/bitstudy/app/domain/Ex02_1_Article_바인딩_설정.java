 ...
 * 2) getter/setter , toString 등의 롬복 어노테이션 사용 / 롬복이 getter setter 를 문장없이? 사용할 수 있게 하는것
...
@Entity /* 엔티티: 정보의 저장 및 전달을 위해서 데이터베이스랑 똑같이 만든 필드
           1) 롬복을 이용해서 클래스를 엔티티로 변경. @Entity 가 붙은 클래스는 JPA 가 관리하게 된다.
...
public class Ex02_1_Article_바인딩_설정 {
    ...
    * */
     ...
     * 그러다가 다시 Set<ArticleComment>를 보고 또 ArticleComment 로 가서 toString 돌리고
     * ,,, 이런식으로 동작하면서 메모리가 터질 수 있다. 그래서 Set<ArticleComment> 에 @ToString.Exclude 을 달아준다.
     *
     * ArticleComment 에 걸지 않고 Article 에 걸어주는 이유는 댓글이 글을 참조하는건 정상적인 경우인데, 반대로
     * 글이 댓글을 참조하는건 일반적인 경우는 아니기 때문에 Article 에 exclude 를 걸어준다.
     *
     **/
 ...
 *
 *      자바에서 2개의 객체를 비교할떄는 equals() 를 사용하는데,
 *      여러 객체를 비교할 때는 equals()를 사용하면 Integer 를 비교하는데 많은 시간이 소요된다.
 *
 * */
